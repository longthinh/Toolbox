...
const qxpng =
	"https://raw.githubusercontent.com/crossutility/Quantumult-X/master/quantumult-x.png"; // server sub-info link
const subinfo_link = {
...
var Ptfo0 =
	mark0 && para1.indexOf("tfo=") != -1
		...
		: 0;
var Prname =
	mark0 && para1.indexOf("rename=") != -1
		? para1.split("rename=")[1].split("&")[0].split("+")
		: null;
var Psrename =
...
var Pcnt =
	para1.indexOf("cnt=") != -1 ? para1.split("cnt=")[1].split("&")[0] : 0;
var Pcap =
	para1.indexOf("cap=") != -1 ? para1.split("cap=")[1].split("&")[0] : "";
var Pptn =
	para1.indexOf("ptn=") != -1 ? para1.split("ptn=")[1].split("&")[0] : ""; //花式英文字符
var Pnptn =
	para1.indexOf("npt=") != -1 ? para1.split("npt=")[1].split("&")[0] : ""; //花式数字
var Pcdn =
	para1.indexOf("cdn=") != -1 ? para1.split("cdn=")[1].split("&")[0] : "";
...
var Pdel =
	mark0 && para1.indexOf("del=") != -1
		...
		: 0; //删除重复节点
var typeU =
	para1.indexOf("type=") != -1 ? para1.split("type=")[1].split("&")[0] : "";
var Pfcr =
	para1.indexOf("fcr=") != -1 ? para1.split("fcr=")[1].split("&")[0] : ""; // force-cellular 等参数
var Pvia =
	para1.indexOf("via=") != -1 ? para1.split("via=")[1].split("&")[0] : ""; // via-interface 参数
var Paead =
	para1.indexOf("aead=") != -1 ? para1.split("aead=")[1].split("&")[0] : ""; // vmess aead 参数
var Phost =
	para1.indexOf("host=") != -1 ? para1.split("host=")[1].split("&")[0] : ""; // host 混淆参数
var Pcsha256 =
	para1.indexOf("csha=") != -1 && version >= 646
		? para1.split("csha=")[1].split("&")[0]
		: ""; // cert-sha256 混淆参数
var Ppsha256 =
	para1.indexOf("psha=") != -1 && version >= 646
		? para1.split("psha=")[1].split("&")[0]
		: ""; // pubkey-sha256 混淆参数
var typeQ = $resource.type ? $resource.type : "unsupported"; //返回 field 类型参数
var PRelay =
	para1.indexOf("relay=") != -1
		? decodeURIComponent(para1.split("relay=")[1].split("&")[0])
		: ""; // 节点 relay 参数, 用于实现代理链功能
var PUOT =
	para1.indexOf("uot=") != -1 && version >= 665
		? para1.split("uot=")[1].split("&")[0]
		: ""; // 节点 udp-over-tcp 开启
var PcheckU =
	para1.indexOf("checkurl=") != -1
	...
	if (flag == 1) {
		...
		if (total.length > 0) {
			if (Psuffix == 1 || Psuffix == -1) {
				total =
					Psuffix == 1
						? total.map(type_suffix)
						: total.map(type_prefix);
			}
			total = total
				.map(type_handle)
				.map(emoji_prefix_handle)
				.map(tag_handle); //各类节点名操作
			if (Psort0) {
				//排序操作
				total = QXSort(total, Psort0);
			}
			total =
				para1.indexOf("node_index_prefix") != -1
					? index_handle(total)
					: total; // 节点序号操作
...
function SubFlow() {
	if (Pinfo == 1 && subinfo) {
		...
		var total =
			"总流量: " +
			(
				parseFloat(sinfo.split("total=")[1].split(",")[0]) /
				1024 ** 3
			).toFixed(2) +
			"GB";
		var usd =
		...
		if (sinfo.indexOf("expire=") != -1) {
			...
			var mth =
				epr.getMonth() + 1 < 10
					? "0" + (epr.getMonth() + 1)
					: epr.getMonth() + 1; // 获取月份(0-11,0代表1月,用的时候记得加上1)
...
function Type_Check(subs) {
	...
	if (subs.indexOf(html) != -1 && link0.indexOf("github.com" == -1)) {
		...
		(/^hostname\s*\=|pattern\=/.test(subi) ||
			RewriteK.some(RewriteCheck)) &&
		...
		type =
			typeQ == "unsupported" || typeQ == "rewrite"
				? "rewrite"
				: "wrong-field"; //Quantumult X 类型 rewrite/ Surge Script/
	} else if (
		...
		// rule/filter类型
		type =
			typeQ == "unsupported" || typeQ == "filter"
				? "Rule"
				: "wrong-field";
		...
		type =
			typeQ == "unsupported" || typeQ == "server"
				? "Subs"
				: "wrong-field"; // QuanX list
	} else if (subs.indexOf("[Proxy]") != -1) {
		type =
			typeQ == "unsupported" || typeQ == "server"
				? "Surge"
				: "wrong-field"; // Surge Profiles
	...
	for (var i = 0; i < Olist.length; i++) {
		...
		if (item.replace(/ /gm, "").indexOf("tag=") != -1) {
			...
			if (nm == "") {
				...
				nm =
					tp + " | " + item.split("=")[1].split(",")[0].split(":")[0];
				item =
					item.split("tag")[0] +
					"tag=" +
					nm.replace("shadowsocks", "ss");
...
function NoReplace(cnt) {
	if (cnt) {
		for (var i = 0; i < 10; i++) {
			cnt = cnt
				.toString()
				.replace(new RegExp(patn[0][i], "gmi"), patn[5][i]);
		}
		return " " + cnt + " ";
	}
}
...
function Capitalize(cnt, para) {
	if (para == 1) {
		cnt =
			cnt.split("tag=")[0] + "tag=" + cnt.split("tag=")[1].toUpperCase();
	} else if (para == -1) {
		cnt =
			cnt.split("tag=")[0] + "tag=" + cnt.split("tag=")[1].toLowerCase();
	} else if (para == 0) {
	...
	if (cnt.indexOf("over-tls=true") != -1 || cnt.indexOf("obfs=wss") != -1) {
		...
		cnt =
			ppsha256 != ""
				? cnt.replace(
						/tag\s*\=/,
						"tls-pubkey-sha256=" + ppsha256 + ", tag="
				  )
				: cnt;
	}
	return cnt;
		...
		try {
			...
			if (!NoteK.some(notecheck)) {
				if (SC.every(sccheck)) {
					...
					subs[i].indexOf(" 302") != -1 ||
					subs[i].indexOf(" 307") != -1
				) {
					//rewrite 302&307 复写
...
function Rewrite_Filter(subs, Pin, Pout, Preg, Pregout) {
	...
	for (var i = 0; i < subs.length; i++) {
		...
		if (
			subi != "" &&
			(subi.indexOf(" url ") != -1 || /^hostname\=/.test(subii))
		) {
...
function HostNamecheck(content, parain, paraout) {
	...
	if (Pntf0 != 0) {
		if (paraout || parain) {
			var noname =
				dname.length <= 10 ? emojino[dname.length] : dname.length;
			var no1name =
				nname.length <= 10 ? emojino[nname.length] : nname.length;
...
function Rule_Handle(subs, Pout, Pin) {
	...
	if (Tout != "" && Tout != null) {
		...
		for (var i = 0; i < cnt.length; i++) {
			...
			if (
				Tout.some(exclude) &&
				!RuleK.some(CommentCheck) &&
				RuleK2.some(RuleCheck)
			) {
		...
		if (Tin != "" && Tin != null) {
			...
			if (nlist.length > 0) {
				var noin0 =
					nlist.length <= 10 ? emojino[nlist.length] : nlist.length;
				if (Pntf0 != 0) {
					$notify(
						"🤖 " + "分流引用  ➟ " + "⟦" + subtag + "⟧",
						"✅ 保留:" + Tin,
	...
	nlist =
		Pfcr == 3
			? nlist
					.filter(Boolean)
					.map((item) => item + ", multi-interface-balance")
			: nlist.filter(Boolean);
	...
	if (Pvia != "") {
		nlist =
			Pvia == 0
				? nlist
						.filter(Boolean)
						.map((item) => item + ", via-interface=%TUN%")
				: nlist
						.filter(Boolean)
						.map((item) => item + ", via-interface=" + Pvia);
	}
...
function Rule_Policy(content) {
	...
	if (RuleK1.some(RuleCheck1) && !RuleK.some(RuleCheck)) {
		...
		if (
			cnt[0].indexOf("URL-REGEX") != -1 ||
			cnt[0].indexOf("PROCESS") != -1
		) {
			nn = "";
		} else {
		...
		if (list0[i].trim().length > 3 && !/\;|\/|\#/.test(list0[i][0])) {
			...
			var tag0 =
				list0[i].indexOf("tag=") != -1
					? list0[i]
							.split(
								/\&*(emoji|udp|tf0|cert|rename|replace)\=/
							)[0]
							.split("tag=")[1]
					: "";
			list0[i] =
				...
				if (type == "vmess" && list0[i].indexOf("remarks=") == -1) {
					...
					if (listi.indexOf("@") != -1) {
						...
						var listh = Base64.decode(
							listi.split("https://")[1].split("#")[0]
						);
						listh =
							"https://" +
							listh +
							"#" +
							listi.split("https://")[1].split("#")[1];
...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
	...
	if (server != "") {
		...
		var cert =
			Pcert0 != 0 ? "tls-verification=true" : "tls-verification=false";
	...
	if (server.indexOf("obfs=") == -1) {
		...
		var host =
			server.indexOf("tls-host") != -1
				? "obfs-host=" +
				  server.split("tls-host=")[1].split(",")[0] +
				  ", "
				: "";
		obfs = obfs + host;
	} else if (server.indexOf("obfs=ws") != -1) {
		obfs =
			server.indexOf("over-tls=true") != -1 ? "obfs=wss, " : "obfs=ws, "; //ws,wss 类型
		var uri =
			server.indexOf("obfs-path=") != -1
				? "obfs-uri=" +
				  server.split("obfs-path=")[1].split('"')[1] +
				  ", "
				: "obfs-uri=/, ";
		obfs = obfs + uri;
		var host =
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
	var server = String(
		Base64.decode(subs.replace("vmess://", "").split("?remarks")[0]).trim()
	).split("\u0000")[0];
	var node = "";
	var ip =
		"vmess=" +
	...
	if (obfs == "none") {
		...
		var ouri =
			subs.indexOf("&path=") != -1
				? subs.split("&path=")[1].split("&")[0]
				: "/"; //ws,wss 类型
		obfs = obfs + "obfs-uri=" + ouri + ", ";
		var host =
			subs.indexOf("&obfsParam=") != -1
				? decodeURIComponent(subs.split("&obfsParam=")[1].split("&")[0])
				: "";
...
function S5R2QX(cnt, tlsp = "false") {
	...
	if (server != "") {
		var ipport =
			"socks5=" + server.split("@")[1].split("#")[0].split("/")[0];
		var uname = "username=" + server.split(":")[0];
		var pwd = "password=" + server.split("@")[0].split(":")[1];
		var tag =
			"tag=" +
			decodeURIComponent(server.split("remarks=")[1].split("&")[0]);
...
function V2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
	...
	var tls13 = PTls13;
	var server = String(
		Base64.decode(subs.replace("vmess://", "")).trim()
	).split("\u0000")[0];
	var nss = [];
	if (server != "") {
	...
	if (jsonl.net == "ws" && jsonl.tls == "tls") {
		...
		uri0 =
			jsonl.path && jsonl.path != ""
				...
				: "obfs-uri=/";
		uri0 =
			uri0.indexOf("uri=/") != -1 ? uri0 : uri0.replace("uri=", "uri=/");
		host0 =
			jsonl.host && jsonl.host != ""
				? "obfs-host=" + jsonl.host + ", "
				: "";
		...
		if (cntt.indexOf("v2ray-plugin") == -1) {
			...
			obfs =
				cnt.split("obfs%3D")[1] != null
					? ", obfs=" +
					  cnt.split("obfs%3D")[1].split("%3B")[0].split("#")[0]
					: "";
			obfshost =
...
function MixArr(cnt, dir) {
	...
	for (i = 0; i < cnt.length - 1; i++) {
		...
		cnt0 =
			dir == 1
				? cnt0.concat(cnt[i])
				: cnt0.concat(cnt[cnt.length - 2 - i]);
	}
	cnt0 =
		dir == 1
...
function emoji_del(str) {
	return str
		.replace(/[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/g, "")
		.trim(); //unescape(escape(str).replace(/\%uD.{3}/g, ''));
}
			...
			if (sname.indexOf(dd[i]) != -1) {
				...
				nname =
					...
					sname
						...
						.trim(); // use regex to remove the original flag
				return [nname, key];
			}
		}
	}
...
function SCT2QX(content) {
	...
	var nserver =
		pobfs != ""
			? "shadowsocks= " +
			  [ipport, pmtd, pwd, pobfs, ptfo, pudp, tag].join(", ")
...
function SSS2QX(content) {
	...
	var pwd =
		"password=" + cnt.split("password")[1].split(",")[0].split("=")[1];
	if (cnt.indexOf("obfs") != -1) {
		pobfs =
			"obfs=" +
			cnt
...
function SVmess2QX(content) {
	...
	var puri =
		paraCheck(cnt, "ws-path") != "false"
			? "obfs-uri=" +
			  cnt.split("ws-path")[1].split(",")[0].split("=")[1].trim()
			: "obfs-uri=/";
	var phost =
...
function LoonSSR2QX(cnt) {
	...
	var obfshost =
		"obfs-host=" + cnt.split(",")[8].replace(/\{|\}/g, "").trim();
	var tag = ", tag=" + cnt.split("=")[0].trim();
	node =
		node + [ip, mtd, pwd, ssrp, ssrpara, obfs, obfshost].join(", ") + tag;
	return node;
}
...
function CSS2QX(cnt) {
	...
	if (obfs.indexOf("websocket") != -1) {
		...
		ohost = cnt["plugin-opts"].host
			...
			: "";
		ouri = cnt["plugin-opts"].path
			? "obfs-uri=" + cnt["plugin-opts"].path
			: "";
		if (obfs == "obfs=wss") {
			// tls verification
	...
	node =
		"shadowsocks=" +
		[ipt, pwd, mtd, udp, tfo, obfs, ohost, ouri, cert, tag]
			.filter(Boolean)
			.join(", ");
	return node;
}
...
function CS52QX(cnt) {
	...
	node =
		"socks5=" +
		[ipt, uname, pwd, tls, cert, tag].filter(Boolean).join(", ");
	//console.log(node)
	return node;
}
	...
	function parser(str) {
		...
		for (var i = 0, len = lines.length; i < len; ++i) {
			...
			if (level > curLevel) {
				...
				if (!added) {
					errors.push(
						"Error: Invalid indentation at line " + i + ": " + line
					);
					return;
				}
			}
				...
				if ((m = line.match(regex["key"]))) {
					...
					if (typeof m[2] != "undefined") {
						...
						if (value[0] == "&") {
							...
							if (currentObj != null)
								currentObj[key] = processLiteralBlock(
									children.shift()
								);
							else
								res[key] = processLiteralBlock(
									children.shift()
								);
						} else if (value[0] == "*") {
							...
							if (currentObj != null)
								currentObj[key] = processFoldedBlock(
									children.shift()
								);
							else
